...
class WatchlistService:
    ...
    def add_item(
        ...
        self._get_owned_watchlist(watchlist_id, user_id)
        if self.asset_repo.get_by_id(data.asset_id) is None:
            raise AppException(status_code=404, detail="종목을 찾을 수 없습니다.")
        if self.item_repo.get_by_watchlist_asset(watchlist_id, data.asset_id):
    ...
    def _get_owned_watchlist(self, watchlist_id: int, user_id: int) -> None:
