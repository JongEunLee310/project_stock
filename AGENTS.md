...
- Before starting work, pull the latest `dev` and create a dedicated feature branch (`feat/<topic>`, or `fix/`/`docs/`/`chore/` by change type) from it. Feature PRs target `dev`, never `main`; `main` is updated only by promoting `dev` through a release PR (see `docs/harness/branch-strategy.md`). Never commit work directly on `dev` or `main`.
