...
- `dev` and `main` are protected: direct push is rejected by repository rules. Push to a feature branch (e.g., `docs/<topic>`, `feat/<topic>`, `fix/<topic>`) branched from the latest `dev`, and open a PR with `gh pr create` targeting `dev` (`--base dev`). `main` is updated only by a `dev` → `main` release PR, which builds and pushes the image. See `docs/harness/branch-strategy.md`.
